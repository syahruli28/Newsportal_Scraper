...
for u in url:
    ...
    if 'detik.com' in u:
        ...
        # TEMPO.CO
        # variabel tambahan
        portaltempo = 'Tempo.co'
        keysearchtempo = key
        # list
        tempoloop = soup.findAll('div', attrs={'class':'gsc-webResult gsc-result'})
        # data yang akan di crawl
        temporawtitle = soup.findAll('a', attrs={'class':'gs-title'})
        temporawrangkuman = soup.findAll('div', attrs={'class':'gs-bidi-start-align gs-snippet'})

        tempo_tambah_satu = open('hasil.csv', 'a', newline='\n')
        # loop
        for x in range(0, len(tempoloop)):
            portal = portaltempo
            search = keysearchtempo
            title = ("{}".format(temporawtitle[x].text.strip()))
            rangkuman = ("{}".format(temporawrangkuman[x].text.strip()))  
            # date = 'Tidak dicantumkan'  
            # menulis ke file csv
            tempo_save_pertama = csv.writer(tempo_tambah_satu)
            tempo_save_pertama.writerow([portal, search, title, rangkuman])
